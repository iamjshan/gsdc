...
var Storage = (function() {
    ...
    function getSurveys() {
        ...
        if (_indexedDBLoaded && _indexedDBSurveys) {
            // 合并数据：IndexedDB（含照片）+ LocalStorage（可能不含照片）
            var merged = Object.assign({}, localSurveys);
            for (var sid in _indexedDBSurveys) {
                // 如果 IndexedDB 中有照片，优先使用 IndexedDB 的数据
                if (_indexedDBSurveys[sid].photos && _indexedDBSurveys[sid].photos.length > 0) {
                    merged[sid] = _indexedDBSurveys[sid];
                }
            }
            return merged;
        }
    ...
    async function getAllSurveysAsync() {
        ...
        try {
            ...
            if (Object.keys(dbSurveys).length > 0) {
                console.log('从 IndexedDB 获取到', Object.keys(dbSurveys).length, '条调查记录');
                // 合并数据：IndexedDB（含照片）+ LocalStorage（不含照片）
                // 优先使用 IndexedDB 的数据（因为它可能包含照片）
                var merged = Object.assign({}, localSurveys);
                for (var sid in dbSurveys) {
                    // 如果 IndexedDB 中有照片，优先使用 IndexedDB 的数据
                    if (dbSurveys[sid].photos && dbSurveys[sid].photos.length > 0) {
                        merged[sid] = dbSurveys[sid];
                    }
                }
                return merged;
            }
    ...
    function saveSurvey(stationId, surveyData) {
        ...
        set('surveys', surveys);
        console.log('Storage.saveSurvey:', stationId);
        console.log('  householdDetails:', JSON.stringify(surveyData.householdDetails));
    }

    // 同步获取单条调查记录（可能不包含照片）
    function getSurvey(stationId) {
        var surveys = getSurveys();
        var survey = surveys[stationId] || null;
        if (survey) {
            console.log('Storage.getSurvey:', stationId);
            console.log('  householdDetails:', JSON.stringify(survey.householdDetails));
            console.log('  photos:', survey.photos ? survey.photos.length : 0, '张');
        }
        return survey;
    }
    
    // 异步获取单条调查记录（包含 IndexedDB 中的照片）
    async function getSurveyAsync(stationId) {
        // 先从 LocalStorage 获取
        var survey = getSurvey(stationId);
        
        // 如果 LocalStorage 中没有照片，尝试从 IndexedDB 获取
        if (!survey || !survey.photos || survey.photos.length === 0) {
            if (typeof IndexedDBStorage !== 'undefined' && IndexedDBStorage.isAvailable()) {
                try {
                    await IndexedDBStorage.init();
                    var dbSurvey = await IndexedDBStorage.getSurvey(stationId);
                    if (dbSurvey) {
                        console.log('从 IndexedDB 获取调查记录:', stationId);
                        // 如果 IndexedDB 中有照片，使用 IndexedDB 的数据
                        if (dbSurvey.photos && dbSurvey.photos.length > 0) {
                            console.log('  从 IndexedDB 获取到', dbSurvey.photos.length, '张照片');
                            return dbSurvey;
                        }
                        // 如果没有照片，但 LocalStorage 也没有，使用 IndexedDB 的数据
                        if (!survey) {
                            return dbSurvey;
                        }
                    }
                } catch(e) {
                    console.error('从 IndexedDB 获取单条调查记录失败:', e);
                }
            }
        }
