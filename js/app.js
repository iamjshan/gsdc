...
(function() {
    ...
    window.viewStationDetail = async function(id, type) {
        ...
        // 尝试从 IndexedDB 获取完整数据（包含照片）
        if (Storage.getSurveyAsync) {
            try {
                var fullSurvey = await Storage.getSurveyAsync(id);
                if (fullSurvey) {
                    // 如果 IndexedDB 中有照片，或者 LocalStorage 中没有调查记录，使用 IndexedDB 数据
                    if ((fullSurvey.photos && fullSurvey.photos.length > 0) || !survey) {
                        console.log('从 IndexedDB 获取到调查记录:', fullSurvey.photos ? fullSurvey.photos.length : 0, '张照片');
                        survey = fullSurvey;
                    }
                }
            } catch(e) {
    ...
    window.nextStep = function() {
        ...
        console.log('下一步: 步骤' + currentStep + ' -> ' + (currentStep + 1));
        console.log('  managementProb:', JSON.stringify(currentSurveyData.managementProb));
        console.log('  householdDetails:', JSON.stringify(currentSurveyData.householdDetails));
    ...
    window.submitSurvey = async function() {
        // 保存最后一步数据（第6步 households 数据）
        var stepData = SurveyForm.collectStepData(currentStep);
        Object.assign(currentSurveyData, stepData);
        
        // 调试信息
        console.log('提交调查数据:', currentStationId);
        console.log('  investigator:', currentSurveyData.investigator);
        console.log('  managementProb:', JSON.stringify(currentSurveyData.managementProb));
        console.log('  householdDetails:', JSON.stringify(currentSurveyData.householdDetails));
