    ...
    function collectStepData(stepNum) {
        ...
        if (hasHouseholdInputs) {
            ...
            for (var i = 0; i < 3; i++) {
                ...
                // 复选框数据在 inputs 循环中已收集，键名为 household_0_selection 等
                var selectionKey = 'household_' + i + '_selection';
                var selections = data[selectionKey] || [];
                var householdData = {
                    name: nameInput ? nameInput.value : '',
                    selection: selections,  // 现在是数组
                    reason: reasonInput ? reasonInput.value : ''
                };
                data.householdDetails.push(householdData);
                console.log('农户' + (i+1) + ':', householdData);
            }
            console.log('收集完成 householdDetails:', JSON.stringify(data.householdDetails));
        } else {
            console.log('非第6步，跳过收集 householdDetails');
        }
        
        // 收集照片数据
        data.photos = currentPhotos;
        
        // 转换"有"/"无"为"是"/"否"（用于保存到数据库）
        ...
        haveNotFields.forEach(function(field) {
            if (data[field] === '有') {
                data[field] = '是';
            } else if (data[field] === '无') {
                data[field] = '否';
            }
        });
        
        // 输出收集到的数据摘要
        console.log('collectStepData 完成，收集到的字段:', Object.keys(data).join(', '));
        console.log('  investigator:', data.investigator || '(未填写)');
        console.log('  problemSummary:', data.problemSummary || '(未填写)');
        console.log('  qualityReport:', data.qualityReport || '(未选择)');
        console.log('  treatmentEquip:', data.treatmentEquip || '(未选择)');
        console.log('  disinfectEquip:', data.disinfectEquip || '(未选择)');
        console.log('  repairInfo:', data.repairInfo || '(未选择)');
        console.log('  managerHealthCert:', data.managerHealthCert || '(未选择)');
        console.log('  repairTimeliness:', data.repairTimeliness || '(未选择)');
