...
var SupabaseClient = (function() {
    ...
    function transformSurveyData(surveyData, stationId) {
        ...
        var dataToSave = {
            ...
            water_quality_prob: surveyData.waterQualityProb || surveyData.water_quality_prob || [],
            management_prob: surveyData.managementProb || surveyData.management_prob || [],
            households: transformedHouseholds,
            household_selections: surveyData.householdSelections || [],  // 旧版简化版农户满意度
            household_details: transformedHouseholdDetails,  // 新版农户详情
            photos: surveyData.photos || []  // 照片URL数组
        };
        
        console.log('  transformSurveyData 设置 household_details:', JSON.stringify(dataToSave.household_details));
        ...
        for (var localField in surveyFieldMapping) {
            ...
            if (surveyData.hasOwnProperty(localField) && surveyData[localField] !== undefined && allowedSurveyFields.indexOf(cloudField) >= 0) {
                ...
                if (localField === 'investigator' || localField === 'problemSummary') {
                    console.log('  映射字段 ' + localField + ' -> ' + cloudField + ':', surveyData[localField]);
                }
            }
        }
        ...
        booleanFields.forEach(function(field) {
            if (dataToSave[field] === '是' || dataToSave[field] === '有') {
                dataToSave[field] = true;
            } else if (dataToSave[field] === '否' || dataToSave[field] === '无') {
        ...
        numericFields.forEach(function(field) {
            if (dataToSave[field] === '' || dataToSave[field] === undefined) {
                ...
                // 确保是数字类型
                var num = parseFloat(dataToSave[field]);
    ...
    function transformCloudToLocal(cloudData) {
        ...
        if (cloudData.station_id) {
            ...
            console.log('  映射 station_id -> stationId:', cloudData.station_id);
        }
        ...
        if (cloudData.household_details !== undefined && cloudData.household_details !== null && Array.isArray(cloudData.household_details)) {
            ...
            console.log('  转换 household_details -> householdDetails:', JSON.stringify(localData.householdDetails));
        } else {
            localData.householdDetails = [];
            console.log('  household_details 为空，设置 householdDetails 为 []');
        }
        ...
        for (var localField in surveyFieldMapping) {
            ...
            if (cloudData.hasOwnProperty(cloudField) && cloudData[cloudField] !== undefined && cloudData[cloudField] !== null && localField !== 'householdDetails') {
                ...
                if (cloudField === 'investigators' || cloudField === 'problem_summary') {
                    console.log('  反向映射字段 ' + cloudField + ' -> ' + localField + ':', cloudData[cloudField]);
                }
            }
        }
    ...
    async function saveSurvey(stationId, surveyData) {
        ...
        console.log('saveSurvey 转换后数据:', stationId);
        console.log('  household_details:', JSON.stringify(dataToSave.household_details));
        console.log('  investigators:', dataToSave.investigators);
        console.log('  照片数量:', dataToSave.photos ? dataToSave.photos.length : 0);
        console.log('  完整数据字段:', Object.keys(dataToSave).join(', '));
        ...
        try {
            console.log('正在保存调查数据到云端:', stationId);
            console.log('  发送的 household_details:', JSON.stringify(dataToSave.household_details));
            ...
            if (error) {
                console.error('保存调查记录失败:', error);
                throw error;
            }
            
            console.log('调查数据保存成功:', data ? data.station_id : 'null');
            console.log('  返回 household_details:', JSON.stringify(data ? data.household_details : 'null'));
